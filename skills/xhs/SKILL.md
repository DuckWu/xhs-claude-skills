...
### 步骤 5：更新索引笔记
在步骤 4 保存笔记后，更新 `<Obsidian 保存目录>/📋 索引.md`。这是一张按标签分组的导航页，方便快速翻阅。
...
> 自动生成，每次提取后更新。按标签分组，扫一眼定位。

## #标签A
- [[2026-05-14 短标题]] — 标题里的核心洞察
- [[2026-05-10 另一个笔记]] — 核心洞察
...
3. 按标签分组，同一标签内的笔记按日期倒序
4. 一条笔记有多个标签时，出现在每个标签分组下
5. 标签也按包含笔记数量降序排列（笔记多的标签在前）
6. 用 Write 写入索引文件

**实现提示：** 可以用 Bash + grep/sed 完成，也可以直接逐个 Read 每个 .md 文件的头几行提取标题和标签，然后构建索引。索引只列笔记文件名（Obsidian `[[wikilink]]` 格式）+ 标题洞察，不要展开详情。

**注意：** 索引文件 `📋 索引.md` 本身不应出现在索引列表中。
